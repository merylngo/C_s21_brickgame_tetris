...
START_TEST(test_1) {
  ...
  ck_assert_msg(!game_state->field, RED_BCGR "function failed" RESET_COLORS);
...
START_TEST(test_2) {
  ...
  ck_assert_msg(!game_state->next_block,
...
START_TEST(test_3) {
  ...
  ck_assert_msg(!game_state->figure.matrix,
                RED_BCGR "function failed" RESET_COLORS);
...
Suite* create_remove_test(void) {
  Suite* suite =
      suite_create(GREEN_BCGR WHITE_FONT ">create_&_remove<" RESET_COLORS);
